...
const ContactPage: React.FC = () => {
  ...
  const handleSend = () => {
    ...
    const emailBody = `Name: ${name}%0AContact Number: ${contactNumber}%0AMessage: ${message}`;
    const emailSubject = encodeURIComponent('New Message from Contact Form');
    const mailtoUrl = `mailto:your-email@hotmail.com?subject=${emailSubject}&body=${emailBody}`;
    window.location.href = mailtoUrl;
  };

  return (
    <div className="h-full overflow-y-auto mx-auto py-8 bg-pink-100 text-gray-800 px-4 md:px-0">
      <div className="flex flex-col md:flex-row items-center justify-center">
        <div className="max-w-md md:mr-8 mb-4 md:mb-0">
          <h1 className="text-3xl md:text-4xl font-bold mb-4">Contact Me</h1>
          <Image src="/ana.jpg" alt="Ana Pic" width={400} height={400} className='rounded-2xl' />
        </div>
        <form className="max-w-md">
          ...
          <button type="button" onClick={handleSend} className="bg-pink-400 hover:bg-pink-600 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">
            Send
          </button>
        </form>
      </div>
